...
export async function runInference(model: Model, weightProvider: WeightProvider, tokenIds: number[], interventions?: Intervention[]): Promise<ActivationCapture> {
  ...
  const norm = cfg.extra.rmsNormVariant === "gemma" ? gemmaRmsNorm : rmsNorm;
  ...
  for (let i = 0; i < cfg.numLayers; i++) {
    ...
    const rms1Out = record(`${b}.rms1`, norm(x, rms1g, eps));
    ...
    if (hasSandwichNorm) {
      const postAttnG = await loadVector(`${L}.post_self_attn_layernorm.weight`);
      attnForResidual = record(`${b}.attn.post_norm`, norm(attnOut, postAttnG, eps));
    }
    ...
    const rms2Out = record(`${b}.rms2`, norm(res1, rms2g, eps));
    ...
    if (hasSandwichNorm) {
      const postMlpG = await loadVector(`${L}.post_mlp_layernorm.weight`);
      ffnForResidual = record(`${b}.ffn.post_norm`, norm(ffnOut, postMlpG, eps));
    }
  ...
  const normOut = record("norm", norm(x, normg, eps));
